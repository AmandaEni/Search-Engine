...
def title_to_info(metadata):
    ...
    for article in metadata:
        title = article[0]
        result[title] = {'author': article[1], 'timestamp': article[2], 'length': article[3]}
    return result
...
def filter_out(keyword, article_titles, keyword_to_titles):
    ...
    for article in article_titles:
        if keyword in keyword_to_titles:
            if not(article in keyword_to_titles[keyword]):
                result += [article]
        else:
...
def articles_from_year(year, article_titles, title_to_info):
    result = []
    todays_date = datetime.date(year, 1, 1)
    unix_timestamp = time.mktime(todays_date.timetuple())
    todays_date_1 = datetime.date(year + 1, 1, 1)
    unix_timestamp_1 = time.mktime(todays_date_1.timetuple())

    for article in article_titles:
        time_of_article = title_to_info[article]['timestamp']
        if (time_of_article >= unix_timestamp) and (time_of_article < unix_timestamp_1):
            result += [article]
    return result
